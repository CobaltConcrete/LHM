...
def run(
    ...
    t_smplx_start = time.perf_counter()
    smplx_params, render_c2ws, render_intrs, render_bg_colors = (
        _extract_driving_pose(driving_path, pose_estimator, timer, device=device)
    )
    t_smplx_total = time.perf_counter() - t_smplx_start
    timer.stamp("driving | TOTAL (pose extraction)", t_smplx_total)

    # ── 5. Override betas with SOURCE person's shape ──────────────────────────
    ...
    # ── 8. Composite ─────────────────────────────────────────────────────────
    with timer.tick("output  | composite + save image"):
    ...
    # ── 9. Total wall time + report ───────────────────────────────────────────
    t_total = time.perf_counter() - t_total_start
    timer.stamp("── WALL CLOCK TOTAL ──", t_total)
