...
eg: 
python inference_videostream5.py --source ./train_data/example_imgs/00000000_joker_2.jpg --driving 0 --output --log --pose-estimator-model ./pretrained_models/human_model_files/pose_estimate/multiHMR_896_L.pt --stream --annotate-pose
"""
...
# ── SMPL-X body-25 skeleton edges (subset visible on torso+limbs) ─────────
# Joint index reference: SMPL-X 127-joint or the 25-joint body subset
# from multiHMR — adjust indices to match your model's joint convention.
_SMPLX_SKELETON_EDGES = [
    # spine / torso
    (0, 3),   # pelvis       → spine1
    (3, 6),   # spine1       → spine2
    (6, 9),   # spine2       → spine3
...
def _draw_pose_annotation(
    img_rgb:      np.ndarray,          # [H, W, 3] uint8 RGB  — will be copied
    joints_2d:    np.ndarray,          # [J, 2]  float32, in *source* image coords
    src_hw:       tuple[int, int],     # (H, W) of the driving frame joints were projected into
    joint_radius: int   = 4,
    bone_thick:   int   = 2,
    alpha:        float = 0.75,        # overlay opacity
) -> np.ndarray:
    """
    Draw a semi-transparent skeleton + joint dots onto `img_rgb`.
    Joints are re-scaled from the source driving-frame resolution to the
    output image resolution so the overlay lines up correctly regardless of
    any resize that happened during rendering.
    """
    out_h, out_w = img_rgb.shape[:2]
    src_h, src_w = src_hw

    scale_x = out_w / src_w
    scale_y = out_h / src_h

    # Scale joints to output resolution
    pts = joints_2d.copy()
    pts[:, 0] *= scale_x
    pts[:, 1] *= scale_y
...
def _render_frame(
    ...
    smplx_params, render_c2ws, render_intrs, render_bg_colors, joints_2d_img, joints_src_wh = pose_result
    ...
    # ── Pose annotation overlay ───────────────────────────────────────────────
    if annotate_pose and joints_2d_img is not None:
        print("[info] Annotating pose overlay on output frame.")
        output_img = _draw_pose_annotation(
            output_img, joints_2d_img,
            src_hw=(joints_src_wh[1], joints_src_wh[0]),  # (H, W) of padded image
        )
    
    elif not annotate_pose:
        print("[info] Pose annotation overlay disabled by user option.")
    elif joints_2d_img is None:
        print("[warn] No joints_2d available for pose annotation overlay.")
...
def run(
    ...
    if is_image:
        ...
        output_img, pose_time, pose_success = _render_frame(
            ...
            src_betas, dtype, device, bg_mode, log, [None], annotate_pose=annotate_pose,
        )
            ...
            while True:
                ...
                with log.tick(f"frame {frame_idx:05d} | total"):
                    ...
                    output_img, pose_time, pose_success = _render_frame(
                        frame_bgr, tmp_path, lhm, pose_estimator,
                        gs_model_list, query_points, transform_mat_neutral_pose,
                        src_betas, dtype, device, bg_mode, log, last_pose, annotate_pose=True,
                    )
...
def _parse_args():
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Draw the estimated skeleton joints/bones as a coloured overlay on the output.",
    )
    return p.parse_args()
...
def main():
    ...
    run(
        ...
        annotate_pose    = args.annotate_pose,
    )
